...
const Coffee = () => {
    ...
    // console.log(coffee);
    const { coffeeName, quantity, supplier, photoURL, taste, details } = coffee;
    return (
        <div className="flex flex-col md:flex-row items-center justify-center py-16 px-5 md:px-0 bg-[#F4F3F0] gap-5 md:gap-28">
            ...
            <div className="font-primary text-xl text-[#1B1A1A] flex flex-col gap-3">
                <h2 className="font-secondary text-5xl text-[#331A15]">{coffeeName}</h2>
                <p><span className="font-semibold">Quantity: </span> {quantity}</p>
                <p><span className="font-semibold">Supplier: </span> {supplier}</p>
                <p><span className="font-semibold">Taste: </span> {taste}</p>
                <p className="max-w-xs"><span className="font-semibold">Details: </span> {details}</p>
            </div>
        </div>
    );
};
